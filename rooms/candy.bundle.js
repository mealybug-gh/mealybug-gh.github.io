...
Candy.Util = function(self, $) {
    ...
    self.Parser = {
        ...
        emotify: function(text) {
            /* Edit: Do not use image emoticons, switch to UTF8. */
            /*var i;
        ...
        Room: {
            ...
            Presence: function(msg) {
                ...
                if (presenceType !== "unavailable") {
                    ...
                    if (nickChange) {
                        // user changed nick
                        var trigger_on_nickchange = function(roomJid, oldNick, newNick) {
                            $(Candy).triggerHandler("candy:core:roster:nickchange", {
                                roomJid: roomJid,
                                oldNick: oldNick,
                                newNick: newNick,
                                oldJid: roomJid + "/" + oldNick,
                                newJid: roomJid + "/" + newNick
                            });
                        };
                        nick = item.attr("nick");
                        var prev_nick = user.getNick();
                        action = "nickchange";
                        user.setPreviousNick(user.getNick());
                        user.setNick(nick);
                        user.setJid(Strophe.getBareJidFromJid(from) + "/" + nick);
                        roster.add(user);
                        trigger_on_nickchange(Strophe.getBareJidFromJid(from), prev_nick, nick);
                    } else {
                        action = "leave";
...
Candy.View.Observer = function(self, $) {
    ...
    self.Chat = {
        ...
        Message: function(event, args) {
            var res = $(Candy).triggerHandler("candy:view.message.before-notice", {
       ...
       */
            showEmoticonsMenu: function(elem) {
                ...
                for (i = Candy.Util.Parser.emoticons.length - 1; i >= 0; i--) {
                    ...
                    emoticons = '<span class="context-menu emoticons items">' + Candy.Util.Parser.emoticons[i].image + "</span>" + emoticons;
                }
...
Candy.View.Template = function(self) {
    ...
    self.Chat = {
        ...
        modal: '<div id="chat-modal"><a id="admin-message-cancel" class="close" href="#">×</a>' + '<span id="chat-modal-body"></span>' + // EDIT: remove image
        '<span class="chat-modal-spinner" id="chat-modal-spinner"></span>' + //'<img src="{{assetsPath}}img/modal-spinner.gif" id="chat-modal-spinner" />' +
